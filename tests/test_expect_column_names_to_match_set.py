...
def test_expect_column_names_to_match_set():
    ...
    assert test_func(row={ "number": 7, "string": "d" }, columns=('number', 'string'), ignore_excess=True)
    assert test_func(row={ "number": 7, "string": "d" }, columns=('number', 'string'), ignore_excess=False)
